...
```{r convert_genes_muraro, eval = TRUE}
...
```{r convert_genes_lawlor, eval = TRUE}
...
```{r load_merged_pancreas, eval = TRUE}
library(MetaNeighbor)
...
```{r compute_global_hvgs, eval = TRUE}
...
```{r compute_global_aurocs, eval = TRUE}
...
_MetaNeighborUS_ returns a cell-type-by-cell-type matrix containing cell type similarities. Cell type similarities are defined as an Area Under the ROC curve (AUROC), which range between 0 and 1, where 0 indicates low similarity and 1 strong similarity.
...
```{r plot_global_aurocs, eval = TRUE, fig.width=7, fig.height=6.5}
...
```{r show_global_top_hits, eval = TRUE}
...
```{r split_global_clusters, eval = TRUE}
...
```{r subset_global_data, eval = TRUE}
...
```{r compute_endocrine_hvgs, eval = TRUE}
...
```{r compute_endocrine_aurocs, eval = TRUE, fig.width=7, fig.height=6.5}
...
```{r split_endocrine, eval = TRUE, fig.width=7, fig.height=6.5}
...
```{r compute_1v1_aurocs, eval = TRUE, fig.width=7, fig.height=6.5}
...
```{r extract_metaclusters, eval = TRUE}
...
```{r plot_upset, eval = TRUE, fig.width=7, fig.height=6.5}
plotUpset(mclusters)
```
...
```{r plot_meta_clusters, eval = TRUE}
...
```{r plot_cluster_graph, eval = TRUE, fig.width=7, fig.height=6.5}
...
```{r plot_subgraph, eval = TRUE, fig.width=7, fig.height=6.5}
coi = "baron|epsilon"
