...
## Generalizable interpretation of cross-dataset similarities

In their computations, MetaNeighbor’s AUROCs are directly related to Spearman correlations. More precisely, all computations are based on average Spearman correlations between cells from two cell types, but include an additional prediction that get rid off batch effects, while keeping an interpretability power that is comparable to correlations (where AUROC = 0 maps to correlation = -1, AUROC = 0.5 maps to correlation = 0, AUROC = 1 maps to correlation = 1).

To appreciate how the additional prediction step enables us to obtain "batch free correlations", we compare MetaNeighbor’s output with a more naive similarity output, where we compute the Spearman correlation between cell type centroids. We start by computing cell type centroids.

```{r centroid_computation}
...
Now we compare centroid correlations with MetaNeighbor AUROCs.
...
Centroid correlations have interesting properties: centroids cluster primarily by cell type (then by dataset), we see global hierarchical structure (MGE-derived interneurons vs CGE-derived interneurons). However, batch effects are clearly visible throughout the heatmap. For example, within each cell type, Chromium-based datasets tend to cluster on one side, SmartSeq datasets on the other side. In contrast, for an equivalent computation time, all the "good" patterns are made exquisitely clear with MetaNeighbor AUROCs.

The conceptual relationship between AUROCs makes cell type similarities interpretable, without having to worry about batch effects. As we have pointed out in Protocol 1, one of the main difference between AUROCs and correlations is the dependency on dataset composition (more precisely, what outgroup is considered in the prediction task). 

```{r average_spearman}
# the following lines are equivalent (but faster) to running
# "mean(cor(cpm, method = "s"))" for each pair of cell type
ranked_cpm = matrixStats::colRanks(cpm, ties.method = "average", preserveShape = TRUE)
scaled_cpm = scale(ranked_cpm) / sqrt(nrow(ranked_cpm)-1)
z_centroids = sapply(levels(cell_types), function(ct) {        
    matrixStats::rowMeans2(scaled_cpm, cols = cell_types == ct)
})
centroid_cor = crossprod(z_centroids)
plotHeatmap((1+centroid_cor)/2, cex = 0.5)
```
